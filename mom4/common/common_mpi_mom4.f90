...
!===============================================================================
  ...
  INTEGER,PARAMETER :: mpibufsize=7258!1000 !600 !(this worked as 'safe' with 480 procs on Gaea) !200 !1000  !STEVE: this fixes the problem of bad output when using over 6 nodes default=1000,mom2(mpich2)=200
...
!===============================================================================
  ...
  WRITE(6,*) "Hello from scatter_grd_mpi."
  WRITE(6,*) "mpibufsize=",mpibufsize,"nij1max=",nij1max
  IF(mpibufsize > nij1max) THEN
    if (dodebug) then
...
!      if (.false.) CALL write_grd4('test.'//filename,v3dg,v2dg) 
    ...
    DO n=0,nprocs-1
      im =INT( n+1 + (l-1)*nprocs)
      IF(im <= nbv) THEN
        WRITE(6,*) "In common_mpi_mom4.f90::read_ens_mpi, calling scatter_grd_mpi..."
        WRITE(6,*) "nlon=",nlon,"nlat=",nlat,"nlev=",nlev,"nv3d=",nv3d,"nv2d=",nv2d,"nbv=",nbv
        WRITE(6,*) "nij1=",nij1,'im=',im
