...
! <<<<<<< HEAD
!   07/08/2015 Modify logic value DO_DRIFTERS = .true.
! =======
...
! ALLOCATE(v3d(nlon,nlat,nlev,nv3d),v2d(nlon,nlat,nv2d))
  ...
  call check( NF90_OPEN(tsfile,NF90_NOWRITE,ncid) )
  WRITE(6,*) "read_grd4:: just opened file ", tsfile

  !!! t
  call check( NF90_INQ_VARID(ncid,'temp',varid) )
  call check( NF90_GET_VAR(ncid,varid,v3d(:,:,:,iv3d_t)) )
  WRITE(6,*) "read_grd4:: just got data for variable t for v3d"
...
! !STEVE: end
  ...
  WRITE(6,*) "read_grd4:: just got data for variable s for v3d"
...
! !STEVE: end
  ...
  call check( NF90_OPEN(uvfile,NF90_NOWRITE,ncid) )
   WRITE(6,*) "read_grd4:: just opened file ", uvfile

  !!! u
  call check( NF90_INQ_VARID(ncid,'u',varid) )
  call check( NF90_GET_VAR(ncid,varid,v3d(:,:,:,iv3d_u)) )
! v3d(nlon,:,:,iv3d_u) = 0.0 !STEVE: why was this?
  WRITE(6,*) "read_grd4:: just got data for variable u for v3d"
...
! v3d(:,nlat,:,iv3d_v) = 0.0 !STEVE: why was this?
  WRITE(6,*) "read_grd4:: just got data for variable v"
...
! !STEVE: end
  ...
  if (.false.) then
    ...
    WRITE(6,*) "read_grd4:: just got data for variable t_surf"
...
! !STEVE: end
    ...
    WRITE(6,*) "read_grd4:: just got data for variable s_surf"
...
! !STEVE: end
  ...
  if (.false.) then
    ...
    WRITE(6,*) "read_grd4:: just got data for variable sea_lev"
...
!===============================================================================
! Write out an letkf grd-format binary file
!===============================================================================
  IMPLICIT NONE
  CHARACTER(*),INTENT(IN) :: filename
  REAL(r_size),INTENT(IN) :: v3d(nlon,nlat,nlev,nv3d)
...
!===============================================================================
! Write out the maximum model grid values for each variable type
!===============================================================================
  IMPLICIT NONE
  REAL(r_size),INTENT(IN) :: v3d(nlon,nlat,nlev,nv3d)
  REAL(r_size),INTENT(IN) :: v2d(nlon,nlat,nv2d)
