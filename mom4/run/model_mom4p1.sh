...
echo "processing cycle: ${IYYYY}${IMM}${IDD}${IHH}"
  ...
  do
    ...
    if [ "$isfirst" -eq "1" ]; then
      ...
      #cp ${root}/OBS/${IYYYY}${IMM}${IDD}${IHH}/${OBS_ENS}/INPUT/drifters_inp.nc ${workdir}/INPUT
      cp ${INPUT_INI}/INIT_DRIFTERS/drifters_inp.${MEM3}.nc ${workdir}/INPUT/drifters_inp.nc

      # turn on the drifters module
      ...
      cp ${EXP_DIR}/${PYYYY}${PMM}${PDD}${PHH}/model/${MEM3}/RESTART/drifters_inp.nc ${workdir}/INPUT/drifters_inp.nc
      #cp /lustre/lysun/models/Ocean-LETKF-gyre1-test3-025/mom4/OUTPUT/1981011500/model/${MEM3}/RESTART/drifters_inp.nc ${workdir}/INPUT/drifters_inp.nc
      ...
      if [ "${ISLOT}" -eq "1" ]; then
        ...
        if [ -e ${workdir_analysis} ]; then
          ...
          if [ -f ${workdir_analysis}/anal${MEM3}.drifters_inp.nc ]; then
            ...
            cp -f ${workdir_analysis}/anal${MEM3}.drifters_inp.nc ${workdir}/INPUT/drifters_inp.nc
             
            #### Copy the perturbation analysis file
            if [ -f ${workdir_analysis}/analp${MEM3}.drifters_inp.nc ]; then 
              echo "Copy perturbation file: analp${MEM3}.drifters_inp.nc..."
              rm -rf ${workdir}/INPUT/drifters_inp.nc
              cp -f ${workdir_analysis}/analp${MEM3}.drifters_inp.nc ${workdir}/INPUT/drifters_inp.nc
            fi
          else
