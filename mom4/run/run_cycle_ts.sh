...
do
  ...
  sh model_mom4p1.sh ${root} ${YYYYMMDDHH} ${days} ${ENS_NUM} ${isfirst} ${OBS_ENS} ${MYYYYMMDDHH} ${ISLOT} ${ddays}
  ...
  if [ "${YYYYMMDDHH}" -ge "${MYYYYMMDDHH}" ]; then
    ...
    if [ "${ISLOT}" -eq "${ddays}" ]; then     
     ...
     sh letkf_ts.sh ${root} ${YYYYMMDDHH} ${days} ${ENS_NUM} ${OBS_ENS} ${NUM_DRIFTERS}     
     # ISLOT=0
    else
