...
press_v_ens_g = np.zeros([nlon-1,nlev,num_ens])
press_v_ens_g2 = np.zeros([nlon-1,nlev,num_ens])
...
for k in range(1,num_ens+1):
    ...
    press_v_ens_g[:,:,k-1] = tmp4[:,ylat,0:nlon-1].transpose()
    press_v_ens_g2[:,:,k-1] = tmp4[:,ylat,1:nlon].transpose()
...
press_v_mean = tmp4[:,ylat,0:nlon-1].transpose()  
press_v_mean2 = tmp4[:,ylat,1:nlon].transpose()
...
dr_v_press_corr_g = np.zeros([2,nlon-1,nlev])
...
ssh_v_press_corr_g = np.zeros([nlon-1,nlev])
