...
echo "processing cycle: ${IYYYY}${IMM}${IDD}${IHH}"
  ...
  mkdir ${EXP_DIR}/${IYYYY}${IMM}${IDD}${IHH}
    ...
    if [ "$isfirst" -eq "1" ]; then
      cp ${INPUT_INI}/${MEM3}/RESTART/input.nml ${workdir}/INPUT/
      cp ${INPUT_INI}/${MEM3}/RESTART/ocean_*.res.nc ${workdir}/INPUT/
         
      cp ${INPUT_INI}/${MEM3}/RESTART/*_table ${workdir}/INPUT/

      NUM_DRIFTERS=`printf %.3d ${NUM_DRIFTERS}`

      cd ${workdir}/INPUT/
      cp ${root_run}/drifters_code/setup_ini_drifters_${NUM_DRIFTERS}.x ./
      ./setup_ini_drifters_${NUM_DRIFTERS}.x 
      cd ${workdir}      
    ...
    # LUYU: For model gyre1
    ln ${INPUT_INI}/${MEM3}/INPUT/grid_spec.nc ${workdir}/INPUT
    ln ${INPUT_INI}/${MEM3}/INPUT/gotmturb.inp ${workdir}/INPUT 
    ln ${INPUT_INI}/${MEM3}/INPUT/tau.nc       ${workdir}/INPUT   
