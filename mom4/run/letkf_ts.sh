...
day2=`printf %.2d ${days}`
cp -f ${root}/obs/NCEP_PROF/d2l_ts_synth_${day2}.x ./
./d2l_ts_synth_${day2}.x #Generating obsin_drifters.dat

if [ -e obsin_drifters.dat ]; then
  echo "obsin_drifters does exist"
else
  echo "obsin_drifters does not exit. ERROR"
  exit 1
fi
...
do 
  ...
  OBSOPexe=obsop.001
  OBSOP_DRexe=obsop.DRIFTERS.ts.001
  echo '*******letkf'
  mkdir -p ${workdir2}
...
cp ${root}/letkf_drifters_ts/$LETKFexe ${workdir2}
cd ${workdir2}
