...
INTEGER :: it_id, nf, nd, ncid, varid
REAL(kind=8), DIMENSION(:), ALLOCATABLE :: intime
REAL(kind=8), DIMENSION(:,:), ALLOCATABLE :: positions
REAL(kind(0.0e0)), DIMENSION(:,:), ALLOCATABLE :: fields
INTEGER, DIMENSION(:), ALLOCATABLE :: index_time, ids, ids_inp
INTEGER :: use_flag
INTEGER, PARAMETER :: outdim = 1000000 ! LUYU: > num_times * num_drifters
REAL(kind(0.0e0)), DIMENSION(outdim) :: lon, lat, depth, temp, salt, time, work
INTEGER, DIMENSION(outdim) :: itime, dids
INTEGER, DIMENSION(outdim) :: didx
INTEGER :: num_drifters, num_times, num_drifters_inp
INTEGER :: outidx = 0
...
CHARACTER(slen) :: input = "INPUT_drifters_inp.nc"
...
do ii=1,np
  WRITE(infile,'(A,I6.6)') 'drifters_out.nc.',ii-1
...
enddo
num_times=itime(didx(2)-1)-itime(didx(1)) + 1
print *, "num_times = ", num_times


!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!LUYU: Somehow for the drifters module in mom4, there might be missing drifters. If the input ids of drifters are not consecutive (e.g. 1,2,4,5), then the output ids would be mislabled. (e.g.1,2,3,4). This additional code is to fix this problem.
  ...
  call check(NF90_INQUIRE_DIMENSION(ncid,varid,len=num_drifters_inp))

  print *, 'finish setting up dim of ids.'

  ALLOCATE(ids_inp(num_drifters_inp))

  call check(NF90_INQ_VARID(ncid,'ids',varid))
  call check(NF90_GET_VAR(ncid,varid,ids_inp))
  print *, 'finish setting up ids_inp.'
...
write(fid,'(8A16)') "ids","lon","lat","depth (m)","temp (ºC)","salt (psu)","time (h)","idx time"
...
if (write_option .eq. 0) then
  ...
  do k=1,j
    ...
    jj = didx(k+1)-1
    do i=j,jj
      ! ***LUYU: somehow, the model mom4p1 didn't correctly scatter drifters to each processer so that will be repeated information for some drifters. Following is an easy way to solve this problem that eliminating the repeated information. We can update this Later..
      if ( (dids(i) .eq. dids(i-1)) .and. (time(i) .eq. time(i-1))) then
        cycle 
      else
        !write(fid,'(I12,6F12.4,I12)') dids(i), lon(i), lat(i), depth(i), temp(i), salt(i), time(i), itime(i)
        depth(i) = 15 ! ***LUYU: we need to chage this value when we test for the Argo drifters or drifters at other depth...
        write(fid,'(I16,4F16.6,E16.4,F16.6,I16)') ids_inp(dids(i)), lon(i), lat(i), depth(i), temp(i), salt(i), time(i)/1800, itime(i) 
       ! ***LUYU: Here "16" needs to be modified if you want to try a larger time period, since variable "time" can occupy more bytes. Or maybe later we should change the scale of variable "time", i.e. in terms of hour.
      endif
    enddo
  enddo
elseif (write_option .eq. 1) then
  ...
  do k=1,j
    i = didx(k+1)-1
    depth(i)=15
    write(fid,'(I16,6F16.6,I16)') ids_inp(dids(i)), lon(i), lat(i), depth(i), temp(i), salt(i), time(i)/1800, itime(i)
  enddo
endif
