...
root_run=${root}/run
#OBS_DIR=${root}/OBS
OBS_DIR=/lustre/lysun/models/Ocean-LETKF-gyre1-test3/mom4/OBS/
EXP_DIR=${root}/OUTPUT/
...
NYYYY=${IYYYY} #`$date -d "$IYYYY-$IMM-$IDD $sinc $sinc_units" +%Y`
NMM=${IMM} #`$date -d "$IYYYY-$IMM-$IDD $sinc $sinc_units" +%m`
NDD=${IDD} #`$date -d "$IYYYY-$IMM-$IDD $sinc $sinc_units" +%d`
NHH=${IHH} #`$date -d "$IYYYY-$IMM-$IDD $sinc $sinc_units" +%H`

OBSDIR=${OBS_DIR}/${NYYYY}${NMM}${NDD}${NHH}/${OBS_ENS}/DRIFTERS
...
########################
cp -f ${root}/run/drifters_code/read_drifters*.f90 ./
sh make_rd.sh
########################

day2=`printf %.2d ${days}`
cp -f ${root}/obs/NCEP_PROF/d2l_prof_synth_${day2}.x ./
./d2l_prof_synth_${day2}.x #Generating obsin_drifters.dat

if [ -e obsin.dat ]; then
  echo "obsin does exist"
else
  echo "obsin does not exit. ERROR"
  exit 1
fi
...
do 
  ...
  #ln -f $workdir_fcst/drifters_inp.nc            gs$ISLOT$MEM3.drifters_inp.nc
...
#  fi
  ...
  #cp ${workdir}/gs$ISLOT${MEM3}.drifters_inp.nc       ${workdir2}/anal${MEM3}.drifters_inp.nc
  ...
  ln -f ${OBSDIR}/obsin.dat obsin.dat
...
LETKFexe=letkf.PROFILE.$MEM3
cp ${root}/letkf/$LETKFexe ${workdir2}
cd ${workdir2}
