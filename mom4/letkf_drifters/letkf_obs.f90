...
!===============================================================================
  ...
  do islot=1,nslots
    ...
    INQUIRE(FILE=obsfile, EXIST=obs_prof_exists)
    if (obs_prof_exists) then
      CALL get_nobs(obsfile,obs2nrec,nobslots(islot))
    else
      WRITE(6,*) 'The profile observation file does not exist'
      nobs = 0
      DO_OCEAN = .false.
      RETURN
    end if
  ...
  nobs = SUM(nobslots)
  WRITE(6,'(I10,A)') nobs,' TOTAL PROF OBSERVATIONS INPUT'
