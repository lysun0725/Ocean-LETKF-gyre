PROGRAM obsop_drifters
  ...
  CHARACTER(slen) :: guesfile = 'gues'	!'gues'!IN data from model space in folder "DRIFTERS"
  ...
  REAL(r_sngl), ALLOCATABLE :: oval(:)  ! value: temp, salt
  ...
  !-----------------------------------------------------------------------------
  CALL get_nobs_drifters(obsinfile,8,nnobs) !Later, need to add time.
  print *, 'nnobs = ', nnobs
  ALLOCATE(  elem(nnobs)  )
  ALLOCATE(  rlon(nnobs)  )
  ALLOCATE(  rlat(nnobs)  )
  ...
  ALLOCATE( oval(nnobs) ) ! extra value for temp, salt 
  print *, 'Finish get_nobs and start read_obs2_drifters.'

  CALL read_obs2_drifters(obsinfile,nnobs,elem,rlon,rlat,rlev,obid,oerr,otime,oval)

  print *, 'Start reading the model by read_drifters'
  !-----------------------------------------------------------------------------
  ...
  DO n=1,nnobs
    ...
    !print *, 'elem(n) = ',elem(n),'obid(n) = ', obid(n), 'ri = ', ri, 'rj = ', rj, 'rk = ', rk
    !print *, obid(n), '=', ri
    ! ri: drifter id index
  ...
  !print *, 'otime=', otime
  CALL write_obs2_drifters(obsoutfile,nnobs,elem,rlon,rlat,rlev,obid,oerr,ohx,oqc,otime,oval)
