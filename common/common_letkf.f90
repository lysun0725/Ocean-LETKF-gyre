...
!     and  C an m by n matrix.
  ...
  if (dodebug) WRITE(6,*) "hdxb^T Rinv hdxb"
  DO j=1,nbv
    DO i=1,nbv
...
!-----------------------------------------------------------------------
  if (dodebug) WRITE(6,*) "hdxb^T Rinv hdxb + (m-1)I/rho" 
  rho = 1.0d0 / parm_infl
  DO i=1,nbv
...
!-----------------------------------------------------------------------
  if (dodebug) WRITE(6,*) "Pa=[ hdxb^T Rinv hdxv + (m-1) I]inv"
  DO j=1,nbv
    DO i=1,nbv
...
!-----------------------------------------------------------------------
  if (dodebug) WRITE(6,*) "T + Pa hdxb_rinv^T dep"
  DO j=1,nbv
    DO i=1,nbv
  ...
  if (dodebug) WRITE(6,*) "MAX(work3(:))=", MAXVAL(work3)
